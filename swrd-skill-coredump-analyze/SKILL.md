...
4. **业务源码证据（硬门禁）**：scene/root_cause/culprit 合计至少引用
   1 处业务源码 `file:line`——`--check` 强制校验，没有则拒绝渲染。
   **"崩溃在 glibc/系统库内部，与业务代码无关"是错误推断，不是跳过
   读码的理由**：malloc/abort/段错误倒在库里，恰恰说明案发机制在业务
   代码（越界写/UAF/坏指针把库弄倒了）——必须经堆指纹、DIE 变量名
   （=g_xxx）、受害结构体（sizeof 反推）把嫌疑指回业务模块，再按
   4a-0 清单读码取证。确实读不到时在 gaps 写明卡点，不许静默跳过。
...
或命令行传参均可。`--config` 可省略——core 就近没有 toml 时引擎自动
读技能包 `workspace/bmccore.toml`）：
...
| `*_evidence.json: source_reading_list` | **读码清单**：哪个文件哪一段+为什么（崩溃链帧/其他线程函数/整读条目） | → 第 4 步按清单打开源码通读（4a-0） |
| `定位结论.conclusions` | 事实条目 + 证据加权根因候选（分值/refs/验证动作） | 这是分析的起点 |
| `根因候选假设` | 每条候选的证据链（指令/寄存器/帧变量/堆布局逐条引用） | 逐条回应的对象（写入 hypotheses） |
...
2. **业务场景还原**：直接使用第 4a-2 步五问的读码结论——触发场景
   （对外接口/线程轮询/定时器/信号/初始化）、共享对象、时序窗口。
   每个线程的角色描述必须是业务语义（"配置热更新下发"而非
   "net_cfg_apply"）。**崩溃帧全在库内（glibc/libc）时**：不许以
   "崩溃在 glibc、与业务代码无关"为由跳过读码——改由堆指纹、
   DIE 变量名、受害结构体把嫌疑指回业务模块，读其分配点/写入口/
   释放路径（DoD 第 4 条硬门禁）
3. **找共享对象**：交叉比对崩溃线程与其他线程的帧、锁地址（优先用
   `=变量名` 形式的 DIE 命名）、堆取证受害对象——找同时出现在两方的
   数据（锁/链表节点/全局缓存/消息队列）
4. **读共享对象的全部读写点**（用 4c 节的搜索模式），确定"谁在什么
   条件下改了它、谁在读它"
5. **组装因果链**：按"A 线程 xxx，但此时 B 线程 xxx，由于 xxx（源码
   机制），导致 xxx（崩溃）"成句；每个分句标注证据出处（节名+地址/行号）
6. **缺口如实声明 + 多候选逐一回应**：证据断在哪就写哪（如"T15
   无符号帧，业务动作不可考"），并列出补什么证据能闭环——不许拿现象
   冒充推断；**对报告"根因候选假设（证据加权排序）"里的每条候选，
   必须在 narrative.hypotheses 逐一回应**：成立→并入叙事主链；
   排除→写明排除依据（排除的假设也是结论）；待验证→给出验证动作
   与执行结果

### 第 4 步：深入源码分析（你的核心价值——必做，禁止跳过）

工具告诉你"崩在哪"，你要告诉用户"为什么"和"怎么修"。
**"关键信息已全部获取，不再需要深入读码"是不合格判断**——引擎证据
（回溯/堆取证/DIE 命名）只完成定位，机制确认必须落到业务源码行；
崩溃点在 glibc/系统库内部时尤其如此（见 DoD 第 4 条：库倒下=
案发在业务码）。

#### 4a-0. 读码总则（什么时候读、读哪里、怎么读）

**什么时候读**：拿到证据包后立刻读——这是你（LLM）的主场，不是 grep 的。
**读哪里**：证据包的 `source_reading_list` 给出了读码清单（哪个文件、
哪一段、为什么值得看），按清单顺序读：
1. 崩溃链各帧（先崩溃函数，再逐个上层调用者）
2. 其他线程的业务函数（多线程场景）
3. 崩溃文件整读条目（入口/调用者/共享数据/生命周期都在这里）

**怎么读**：用你自己的文件读取能力**打开源码文件通读**——从函数声明
读到闭括号，必要时读整个文件。grep 只用于 4c 节的定位取证（搜指纹/
搜赋值点）；**业务理解必须通读，禁止用 grep 结果冒充读过代码**。
你是资深开发：读函数时脑子里装着"谁在什么业务场景调它"，
而不是只看函数体本身。

#### 4a-1. 读取崩溃帧的完整函数

工具只贴 ±5 行片段。你需要读整个函数（见 4a-0 清单）。

**关键问题**：
- 崩溃行的变量从哪来的？（参数？全局？返回值？）
- 谁调用了这个函数？调用链上有没有条件分支导致异常路径？
- 这个函数持有的资源（锁/引用/内存）在崩溃时的状态？
- 上层调用者逐个读：崩溃线程回溯/线程现场还原里的每个上层帧都要
  打开读——异常数据的入口（谁把坏值造出来/传进来的）往往在调用链
  上游，只读崩溃帧永远追不到案发现场

#### 4a-2. 业务场景还原（五问——叙事的骨架素材）

通读源码后回答以下五问，答案直接进 narrative 的 scene/thread_roles/
root_cause。**写业务语义，禁止复述函数名**——"崩在 net_cfg_apply"
不合格，"配置热更新线程批量下发时与状态采集线程锁序交叉"才合格。

1. **什么场景触发的？** 沿调用链向上读到业务入口，分类：对外接口
   （DBus/IPMI/HTTP/命令行）/ 线程轮询 / 定时器 / 信号回调 / 初始化
   流程 / 回调注册。证据：入口函数（main/pthread_create 的线程函数/
   注册点），标注 file:line
2. **共享了什么？** 崩溃路径与其他线程/路径共用哪些数据（锁、链表、
   全局缓存、队列）？谁写谁读？交叉比对：锁地址命名（=g_xxx）、
   堆对象还原、其他线程的帧
3. **指针生命周期？** 崩溃指针从哪来（分配点/赋值点/传参路径）到哪去
   （释放点/置空点）？结合帧变量运行时值判断"断在哪一环"
4. **时序窗口？** 崩溃点前后的时序假设是什么？哪两个动作本应有序
   （锁/引用计数/屏障）却可能交错？
5. **修复影响面？** 同一函数/同一共享数据的其他使用方有哪些？
   修复会不会改变他们的行为（这决定 fixes 的谨慎程度）
...
以叙事还原案发时刻：**每个线程在干什么、持有什么、等什么**，多线程必须
写清交叉时序，句式示例（证据逐句标注来源节）：

> **T12（崩溃线程）** 正在 `sensor_read()` 遍历链表（pc=sensor.c:88，
> 见"回溯"节）；此前 **T15** 持锁 0x7f42 在 `db_compact()` 里释放了
> 链表节点（栈顶 db_compact.c:210，持锁证据见"线程现场还原"节）。
> 由于 T15 的释放发生在 T12 遍历路径之外、且读侧无锁保护
> （sensor.c:80），**导致** T12 读到已释放节点的 next=0x5858... → SIGSEGV。

单线程场景则还原"数据从合法变成非法"的完整路径：声明 → 赋值 → 被改
（谁改的）→ 崩溃点使用。报告的"线程现场还原"节（每线程顶部帧+锁关系）
是你组织这段叙事的底稿。
...
8. **禁止复述现象**——"访问了非法地址/空指针"不是结论。必须回答：谁、
   在什么路径上、因为什么机制，把什么东西变成了这样；多线程必须给出
   "A 线程 xxx，但此时 B 线程 xxx，由于 xxx，导致 xxx"的因果链。
   证据不足以支撑时就如实写"卡在 X，需要 Y"——不许拿现象冒充根因
