...
**`swrd-skill-coredump-analyze/` 目录就是一个自包含技能包**：`SKILL.md` 为入口，
...
python3 tools/export_skill.py     # 产出 dist/swrd-skill-coredump-analyze/ 和 dist/*-v3.20.0.zip
unzip dist/swrd-skill-coredump-analyze-v3.20.0.zip -d ~/.codebuddy/skills/
```
...
```bash
cd ~/cores
cp ~/Coredump/swrd-skill-coredump-analyze/bmccore.toml.example bmccore.toml
