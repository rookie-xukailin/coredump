...
全部填绝对路径；含工具链节）——**经 use_skill 调 swrd-skill-writer 写入**，
...
### 5. 产出叙事 HTML 报告（完成标准：四条全达成才算完成）

**DoD**：① narrative.json 过 `render_report.py --check` 校验；② HTML 真实
生成到 workspace/report/；③ 对话内已给 TL;DR+叙事全文+HTML 路径；
④ **业务源码证据**——scene/root_cause 至少引用 1 处业务 `file:line`
（--check 硬门禁）。**"崩溃在 glibc 内部、与业务代码无关"是错误推断**：
库倒下恰恰说明案发在业务码（越界写/UAF/坏指针），必须经堆指纹/DIE
变量名/受害结构体把嫌疑指回业务模块并读码。任何一步失败必须说明并
重试，不许静默只交 md。

按 SKILL.md"叙事构建六步法"组装结果（线程角色表→逐线程业务动作→找
共享对象→读读写点→A/B 因果链→缺口声明），**经 use_skill 调
swrd-skill-writer** 写成 `narrative.json`：
...
6. 崩溃线程≠肇事线程——多线程先看"线程现场还原"的持锁/等锁交叉，
   再沿调用链上溯读源码，追出把数据变坏的那一方
